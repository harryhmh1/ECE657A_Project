...
# sys.stdout = log_file
le = LabelEncoder()
raw_data = pd.read_csv("Albuquerque.csv", names=["date_time", "temperature", "humidity", "pressure", "wind_speed", "weather"])

y = raw_data.loc[119:, ["weather"]]
X = raw_data.loc[:43830, ["temperature", "humidity", "pressure", "wind_speed"]]
...
model.add(kr.layers.Dense(units=37, kernel_initializer=init, activation='softmax'))
...
plot_model(model, to_file='model_Albuquerque.png', show_shapes=True)
...
plt.savefig("acc_consequence_Albuquerque")
plt.show()
...
plt.xlabel('Epoch')
plt.legend(['Train', 'Test'], loc='upper left')
plt.savefig("loss_consequence_Albuquerque")
plt.show()