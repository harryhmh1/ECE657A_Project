...
# sys.stdout = log_file
le = LabelEncoder()
raw_data = pd.read_csv("seattle2.csv", names=["date_time", "hour", "temperature", "humidity", "pressure", "wind_speed", "weather"])

y = raw_data.loc[119:, ["weather"]]
X = raw_data.loc[:44756, ["hour", "temperature", "humidity", "pressure", "wind_speed"]]
...
model.add(kr.layers.Dense(units=20, kernel_initializer=init, activation='relu'))
...
plt.savefig("acc_consequence_seattle2")
plt.show()
...
plt.xlabel('Epoch')
plt.legend(['Train', 'Test'], loc='upper left')
plt.savefig("loss_consequence_seattle2")
plt.show()
