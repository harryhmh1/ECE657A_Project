...
# sys.stdout = log_file
le = LabelEncoder()
raw_data = pd.read_csv("toronto2.csv", names=["date_time","hour", "temperature", "humidity", "pressure", "wind_speed", "weather"])

y = raw_data.loc[119:, ["weather"]]
X = raw_data.loc[:43426, ["hour", "temperature", "humidity", "pressure", "wind_speed"]]
# y = raw_data[["weather"]]
# X = raw_data[["temperature", "humidity", "pressure", "wind_speed"]]
...
model.add(kr.layers.Dense(units=30, kernel_initializer=init, activation='relu'))
model.add(kr.layers.Dense(units=20, kernel_initializer=init, activation='linear'))
model.add(kr.layers.Dense(units=11, kernel_initializer=init, activation='softmax'))
...
plot_model(model, to_file='model_toronto_3.png', show_shapes=True)
...
plt.savefig("acc_consequence_toronto_3")
plt.show()
...
plt.xlabel('Epoch')
plt.legend(['Train', 'Test'], loc='upper left')
plt.savefig("loss_consequence_toronto_3")
plt.show()